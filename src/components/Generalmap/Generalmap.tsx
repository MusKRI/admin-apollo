...
const Generalmap: React.FC<GeneralMapProps> = ({ value }) => {
  ...
  const handleSubmit = async (values: any, {}: any) => {
    ...
    const bodyContent = {
      ...
      meta_data: {
        ...
        ...completeMediaData,
        ...(PageName === "Home" ? { people_and_planet: {} } : {}),
      },
    };
    console.log("body_meta_data", bodyContent);
    console.log("token", localStorage.getItem("apollo-token"));
  ...
  console.log("payload", PayLoadBody);

  let apiIndex = value?.[0].Section1?.[0].Page;
  let endPoint = PayLoadBody[apiIndex].slug;

  useEffect(() => {
    console.log(
      "fetch url",
      `https://webwila.com/giftopedia/public/api/v1/page/${endPoint}`
    );
    axios
      .get(`https://webwila.com/giftopedia/public/api/v1/page/${endPoint}`, {
                    ...
                    <div className="card">
                      <div className="card-body">
                        <div key={index} className="row justify-content-center">
                          <h1 className="col-md-3 d-flex justify-content-center ">
                            {self.Heading}
                          </h1>
